...
public class RegexEditText extends AppCompatEditText implements InputFilter {
    ...
    public RegexEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.common_RegexEditText);

        if (array.hasValue(R.styleable.common_RegexEditText_common_inputRegex)) {
            setInputRegex(array.getString(R.styleable.common_RegexEditText_common_inputRegex));
        } else if (array.hasValue(R.styleable.common_RegexEditText_common_regexType)) {
            int regexType = array.getInt(R.styleable.common_RegexEditText_common_regexType, 0);
            switch (regexType) {
                case 0x01:
