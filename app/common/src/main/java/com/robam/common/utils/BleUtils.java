...
public class BleUtils {
    private static final byte FRAME_HEAD = (byte)0xFE;//帧头
    private static final byte FRAME_HEAD_SEND = (byte)0x5C;//帧头
    private static final int ENCRYPT_RAND_CODE_LEN = 1; //!加密字段占用长度

    private final static byte[] gc_encrypt_table = {
    ...
    private final static byte[] gc_decrypt_table = {
            (byte) 0x48, (byte) (byte) 0x44, (byte) 0x4b, (byte) 0x46, (byte) 0x40, (byte) 0x4e, (byte) 0x42, (byte) 0x4c, (byte) 0xc1, (byte) 0xcf, (byte) 0xca, (byte) 0xcd, (byte) 0xc3, (byte) 0xc9, (byte) 0xc7, (byte) 0xc5,
    ...
    public static void decode(byte[] data) {
        ...
        if (null == data || data.length == 0)
            return;
        if (data[0] != FRAME_HEAD || data[1] != FRAME_HEAD_SEND)
            return;
        pos += 2;
        byte option = data[pos];
        pos++;
        short length = data[pos];
        if (length > 0x7F) {
            pos++;
            byte length2 = data[pos];
            if (length2 > 0x7F)
                return;
            length = (short) (length | (ByteUtils.toShort(length2) << 7));
        }
        pos++;
        for (int i=pos; i < data.length; i++)
            data[i] = gc_decrypt_table[data[i]];  //解密
        byte ran_code = data[pos];
        byte[] targetData = new byte[length];
        for (int i=0; i < length - ENCRYPT_RAND_CODE_LEN; i++)
            targetData[i] = (byte) (data[pos + i + ENCRYPT_RAND_CODE_LEN] ^ ran_code); //异或
    }
}
