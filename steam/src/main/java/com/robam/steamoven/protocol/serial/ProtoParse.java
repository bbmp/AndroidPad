...
public class ProtoParse {
    ...
    // CRC 高8位查表
    static final short[] c_crc_htalbe = {
    ...
    // CRC校验查表低8位
    static final short[] c_crc_ltalbe =	{
    ...
    private PARSE_STATUS parseStatus = PARSE_STATUS.PARSE_STATUS_START;
    private byte payloadIdx = 0;
    private byte[] payload;


    //功能选择打包命令 1.2协议对应长度
    public static byte[] packCtrlCmd(byte[] data) {
    ...
    public static byte[] packCtrTest(byte[] data) {
        byte[] result = new byte[29];
        result[0] = FRAME_HEAD_SEND;
        result[1] = 28;//len
        result[2] = MSG_TYPE_CMD;
    ...
    //功能查询打包命令
    public static byte[] packPoolCmd() {
        byte[] result = new byte[10];
        result[0] = FRAME_HEAD_SEND;
        result[1] = 9;//len
        result[2] = MSG_TYPE_POOL;
        result[3] = VERSION;
        short crc = Crc16Utils.calcCrc16(result, 1, result.length - 3);
        result[result.length - 2] = (byte)(crc & 0xff);
        result[result.length - 1] = (byte)((crc >> 8) & 0xff);
