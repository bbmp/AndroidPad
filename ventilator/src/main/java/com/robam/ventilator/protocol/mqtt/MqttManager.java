...
public class MqttManager {
    ...
    private IMqttActionListener iMqttActionListener = new IMqttActionListener() {
        ...
        public void onSuccess(IMqttToken arg0) {
            Log.i(TAG, "连接成功 ");
            try {
                String topic = new RTopic(RTopic.TOPIC_UNICAST, VentilatorFactory.getPlatform().getDt()
                        , VentilatorFactory.getPlatform().getMac()).getTopic();
...
//            Log.i(TAG, "订阅失败 " + exception.getMessage());
    ...
    private MqttCallback mqttCallback = new MqttCallback() {
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            ...
            int msgId = VentilatorFactory.getProtocol().decode(topic, message.getPayload());
...
//                mqttMsgCallback.messageArrived(message.getPayload());
    ...
    public void publish(MqttMsg msg) {
        ...
        Boolean retained = false;
        try {
            byte[] data = VentilatorFactory.getProtocol().encode(msg);
    ...
    private void deviceConnectedNoti() {
        Log.i(TAG, "发送设备上线通知  " + android.os.Process.myTid() + "  " + VentilatorFactory.getPlatform().getMac());
        MqttMsg msg = new MqttMsg.Builder()
                .setMsgId(MsgKeys.DeviceConnected_Noti)
                .setDt(VentilatorFactory.getPlatform().getDt())
                .setSignNum(VentilatorFactory.getPlatform().getMac())
                .setTopic(new RTopic(RTopic.TOPIC_BROADCAST, VentilatorFactory.getPlatform().getDt(), VentilatorFactory.getPlatform().getMac()))
                .build();
